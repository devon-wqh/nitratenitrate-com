...
async function statsPage(env, url) {
  ...
  const views = await one("SELECT count(*) AS n FROM events WHERE type='view'");
  const clicks = await one("SELECT count(*) AS n FROM events WHERE type='click'");
  const visits = await one("SELECT count(DISTINCT visitor) AS n FROM events");
...
<style>
  body{font-family:"EB Garamond",Georgia,serif;margin:2rem;max-width:640px;color:#000}
  ...
  td{padding:.25rem .5rem;border-bottom:1px solid #eee} td:last-child{text-align:right;width:5rem}
</style></head><body>
<h1>Nitrate — all-time stats</h1>
<div class="big">
